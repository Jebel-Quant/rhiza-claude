...
| `_issue_*` | `issue_status.py`'s two halves. `_issue_signals` is the text half — what an issue's prose says about whether it can be fixed without a judgement call; `_issue_forge` is the forge half — the `gh`/`glab` argv, the subprocess, and normalising both CLIs' payloads into one shape. Split because asking a forge for a payload and reading regexes over English are different instruments with different failure modes, and only the second can be covered exhaustively from doctests |
...
| `_make_targets_*` | `check_make_targets.py`'s halves, split by instrument rather than by size. `_make_targets_runner` is the **delegation** half — is this makefile a shim, what does it pin, what does the pinned `rhiza-task` offer, and what to advise a caller who finds one. `_make_targets_chain` is the **static** half — which makefile, the files it `include`s, the targets they document — and runs nothing. The orchestrator keeps the `make -n` probing, because what resolves and what a file says are different answers. Neither helper imports the orchestrator: the runner is handed the chain, and the chain module is what hands it over |
