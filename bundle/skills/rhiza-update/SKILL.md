...
> **Portable copy of `plugin/skills/update/SKILL.md`, generated by
...
> - **Arguments:** [version e.g. v1.2.0]  (optional; defaults to the template's latest release).
>   The text below writes `$ARGUMENTS` for what the user passed;
>   substitute it yourself if your client does not.
> - **Runs:** `git`, `gh`, `glab`, `uv`, `cat`, `grep`. Nothing here enforces that list, where the
...
## 1. Preconditions
- **`uv` first.** `Read` **`${RHIZA_ROOT}/bundle/prompts/install-uv.md`** and follow
  it before anything else (in a source checkout, `bundle/prompts/install-uv.md`). It's a
...
**`${RHIZA_ROOT}/bundle/prompts/pr-base.md`** and follow it with
...
## 5. Sync
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/sync.py" .
```
(**Keep the quotes.** In a source checkout `${RHIZA_ROOT}/plugin` is empty — fall
...
uv run --python 3.12 --no-project python \
    "${RHIZA_ROOT}/plugin/scripts/resolve_conflicts.py" .
```
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/stage_synced.py" .
```
