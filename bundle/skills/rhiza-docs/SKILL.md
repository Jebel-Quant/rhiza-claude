...
> **Portable copy of `plugin/skills/docs/SKILL.md`, generated by
...
> - **Arguments:** [readme | claude | mkdocs | all]  (optional; defaults to all).
>   The text below writes `$ARGUMENTS` for what the user passed;
>   substitute it yourself if your client does not.
> - **Runs:** `git`, `gh`, `glab`, `grep`, `find`, `cat`, `head`, `make`, `uv`. Nothing here enforces that list, where the
...
- **Default branch + visibility** — one call, both platforms:
  ...
  uv run --python 3.12 --no-project python \
    "${RHIZA_ROOT}/plugin/scripts/platform_cli.py" repo-view --json
  ```
...
`${RHIZA_ROOT}/plugin` is empty, so use the repo-relative path):
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/render_badges.py" \
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/sync_readme_help.py" .
```
