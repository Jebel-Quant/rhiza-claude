...
> **Portable copy of `plugin/skills/fix/SKILL.md`, generated by
...
> - **Arguments:** [issue numbers to scope to, or --dry-run to triage and stop]  (optional; defaults to every open issue).
>   The text below writes `$ARGUMENTS` for what the user passed;
>   substitute it yourself if your client does not.
> - **Runs:** `uv`, `uvx`, `gh`, `glab`, `git`, `make`. Nothing here enforces that list, where the
...
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/platform_cli.py" auth-status
git status --porcelain
```

`${RHIZA_ROOT}/plugin` resolves at runtime (**keep the quotes**); in a source checkout it is
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/check_make_targets.py" --json
```
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/issue_status.py" --json
```
...
1. **Branch.** `Read` `${RHIZA_ROOT}/bundle/prompts/pr-base.md` and follow it with
...
6. **Push and open.**
   ...
   ```
   ```bash
   uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/platform_cli.py" pr-create --base "$DEFAULT" --head "$BRANCH" --title "fix: drop the dead pytest table (#95)" --body-file /tmp/body.md
   ```
