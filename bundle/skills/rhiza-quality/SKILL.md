...
> **Portable copy of `plugin/skills/quality/SKILL.md`, generated by
...
> - **Arguments:** [path or topic to scope the assessment to]  (optional; defaults to the whole repo).
>   The text below writes `$ARGUMENTS` for what the user passed;
>   substitute it yourself if your client does not.
> - **Runs:** `make`, `git`, `gh`, `glab`, `uv`, `uvx`, `python3`, `grep`, `find`, `wc`, `sed`, `sort`, `uniq`. Nothing here enforces that list, where the
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/language_profile.py" --json
```
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/check_make_targets.py"
```
...
- **Enumerate the real tasks first**, from the runner rather than from make. The probe
  ...
  ```bash
  uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/check_make_targets.py" --tasks
  ```
...
Present? `Read` **`${RHIZA_ROOT}/bundle/prompts/quality-charter.md`** and follow it (in
a source checkout, `bundle/prompts/quality-charter.md`), then read the charter itself.
...
8. **Docstring examples and README fences** — run the bundled checker
   ...
   ```bash
   uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/check_doc_examples.py" --source-root <SOURCE_ROOT> --json
   ```
...
- **If a gate fails, `Read`
  `${RHIZA_ROOT}/bundle/prompts/known-issues.md` before diagnosing** (in a source
  checkout, `bundle/prompts/known-issues.md`). Some failures are upstream and unsatisfiable
...
the synced test files themselves is upstream — and `bundle/prompts/known-issues.md` names the
...
`Read` **`${RHIZA_ROOT}/bundle/prompts/design-analysis.md`** and follow it (in a
source checkout, `bundle/prompts/design-analysis.md`). Complexity and architecture are the two
...
`Read` **`${RHIZA_ROOT}/bundle/prompts/scorecard.md`** and follow it. It owns the
