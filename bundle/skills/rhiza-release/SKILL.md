...
> **Portable copy of `plugin/skills/release/SKILL.md`, generated by
...
> - **Arguments:** [version e.g. v1.4.0]  (optional; omit to pick from a table of candidates).
>   The text below writes `$ARGUMENTS` for what the user passed;
>   substitute it yourself if your client does not.
> - **Runs:** `git`, `gh`, `glab`, `uv`, `uvx`, `make`, `cat`, `grep`. Nothing here enforces that list, where the
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/check_version_bump.py" \
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/wait_for_merge.py" \
