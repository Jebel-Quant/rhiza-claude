...
> **Portable copy of `plugin/skills/status/SKILL.md`, generated by
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/validate.py" $ARGUMENTS
```
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/status.py" $ARGUMENTS
```
