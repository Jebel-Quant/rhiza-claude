...
> **Portable copy of `plugin/skills/remote/SKILL.md`, generated by
...
> - **Arguments:** [branch, or --all for every open request]  (optional; defaults to the branch you are on).
...
`${RHIZA_ROOT}/plugin` resolves at runtime (**keep the quotes**); in a source checkout
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/pr_status.py"
```
...
4. **A known upstream failure.** In a rhiza-managed repo, `Read`
   `${RHIZA_ROOT}/bundle/prompts/known-issues.md` (source checkout:
   `bundle/prompts/known-issues.md`) before diagnosing. It names failures that are not this
