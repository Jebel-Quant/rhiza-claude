> **Portable copy of `plugin/prompts/skeleton.md`, generated by
...
- **`uv`** — `/init` has already followed `bundle/prompts/install-uv.md` by the time you get
  here, so `uv` is present. If `uv --version` somehow fails, `Read`
  `${RHIZA_ROOT}/bundle/prompts/install-uv.md` and follow it, then stop if it still
  fails.
...
Invoke the bundled script with the plugin-root path (`${RHIZA_ROOT}/plugin` resolves
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/init_skeleton.py" . \
...
`Read` **`${RHIZA_ROOT}/bundle/prompts/python-version.md`** and follow it with
`$PYTHON_VERSION` (in a source checkout, `bundle/prompts/python-version.md`). It pins
...
**The license is not this procedure's job either.** `bundle/prompts/license.md` owns the
...
edits are idempotent and additive, the license is `bundle/prompts/license.md`'s job, and the
...
idempotent and additive, the licence is `bundle/prompts/license.md`'s job, and the gate in G4 is
not optional.
...
- **No license classifiers — ever.** Neither this procedure nor `bundle/prompts/license.md`
