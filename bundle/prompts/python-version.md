> **Portable copy of `plugin/prompts/python-version.md`, generated by
...
> scan for commands, so the user cannot invoke it. `bundle/prompts/skeleton.md` reads it and follows it at its
...
```bash
uv run --python 3.12 --no-project python "${RHIZA_ROOT}/plugin/scripts/set_python_version.py" . \
