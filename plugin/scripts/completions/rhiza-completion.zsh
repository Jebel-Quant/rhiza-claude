#compdef make
# Zsh completion for make targets, shipped with the rhiza Claude Code plugin.
#
# Install it with `/rhiza:completions` (or `--shell zsh`), which copies this file to
# ${XDG_DATA_HOME:-$HOME/.local/share}/zsh/site-functions/_make.
#
# To wire it up by hand instead, put it on your fpath as `_make` and call compinit:
...
#
# Nothing here is repo-specific — targets are discovered by parsing the make database
# in the current directory, so this completes any make project. The cache key includes
# rhiza's own makefile fragments so a sync that changes them invalidates it; outside a
# rhiza-managed repo those globs simply match nothing.
